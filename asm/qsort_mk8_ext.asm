...
sort:
    ...
    # if length(array) > 1
    load right sub left cmp 1 jlt sort_end jeq sort_end
    # select first element as the pivot
    peek left to pivot
    # while left <= right
    loop1: load left cmp right jgt loop1_end
      # while array[left] < pivot
      loop2: peek left cmp pivot jgt loop2_end jeq loop2_end
        inc left
        jmp loop2 loop2_end:
      # while array[right] > pivot
      loop3: peek right cmp pivot jlt loop3_end jeq loop3_end
        dec right
        jmp loop3 loop3_end:
      # if left <= right
